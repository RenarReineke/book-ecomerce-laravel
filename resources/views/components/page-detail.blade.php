...
<div class="p-10 pt-4 px-5 h-full w-full bg-sky-50 overflow-y-auto">
    <!-- Верхняя панель -->
    <div class="flex justify-between">
        <!-- Заголовок -->
        <h1 class="w-1/8 pb-2 text-gray-700 bg-sky-50 flex justify-start items-center">
            <!-- Ссылка на домашняя страницу -->
            <a href="{{ route('dashboard') }}">
                <svg class="mr-1 w-6 h-6 text-indigo-600 hover:text-indigo-800" fill="none" viewBox="0 0 24 24"
            ...
            <span>
                <svg class="mx-1 w-3 h-3 text-slate-600 stroke-2" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor">
            ...
            <a href="{{ $listLink }}" class="flex items-center">
                <svg class="mr-1 w-5 h-5 text-indigo-600 hover:text-indigo-800" fill="none" viewBox="0 0 24 24"
                    stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <!-- Действия -->
            <x-action-list editLink="{{ $editLink }}" deleteLink="{{ $deleteLink }}"></x-action-list>
        </h1>
        <!-- Заголовок конец -->

        <!-- Кнопка показа панели фильтров -->
        <button id="filterButtonUp"
