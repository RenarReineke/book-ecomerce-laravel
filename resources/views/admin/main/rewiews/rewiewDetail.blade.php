@extends('admin.layouts.dashboard') @section('rewiewDetail')
    <x-page-detail title="Отзыв {{ $rewiew->id }}" listTitle="Отзывы"
        ...
        <h1 class="mb-3 w-fulltext-center text-lg text-gray-700 flex justify-center items-center">
            Отзыв </span> клиента
            <span class="mx-1 font-bold capitalize">{{ $rewiew->client->name }}</span>
            на товар
            <span class="ml-1 font-bold">№ {{ $rewiew->product->id }}</span>
            <span class="ml-1 font-bold capitalize">{{ $rewiew->product->title }}</span>
        </h1>
        ...
        <div>
            @foreach ($rewiew->images() as $image)
                @if ($image)
                    <img src="{{ asset('storage/' . $image->url) }}" alt="Фото от юзера"
        ...
        <div class="mx-auto w-1/2 text-md space-y-8 bg-white p-4 rounded-lg">
            ...
            <div class="shadow-lg rounded-lg p-4 pt-1 flex justify-between">
                <p class="font-medium text-slate-900">
                    Лайки:
                    <span class="ml-1 text-slate-500">{{ $rewiew->likes->count() }}</span>
                </p>
                <p class="font-medium text-slate-900">
                    Оценка товара:
                    <span class="mx-1 text-slate-500">{{ $rewiew->rating }} из 5</span>
                </p>
            </div>
            ...
            <div class="flex flex-col items-end space-y-2">
                <h6 class="font-medium text-slate-900 mr-64">
                    ...
                    <form method="get" action="{{ route('comments.create') }}" class="mx-auto w-12">
                        ...
                        <input type="hidden" name="commentable_id" value="{{ $rewiew->id }}" />
                        <button type="submit" class="h-full w-full">
                            <svg class="w-8 h-8 text-sky-700/30 hover:text-sky-700" viewBox="0 0 24 24" fill="currentColor">
                ...
                <ul class="w-[500px] bg-white divide-y divide-slate-200">
                    @foreach ($rewiew->comments as $comment)
                        <li class="p-4">
                            ...
                            <div class="rounded-t-lg flex justify-between bg-slate-700 p-1 px-2">
                                <div class="flex">
                                    <img src="{{ Vite::asset('resources/images/avatar.jpg') }}" alt="Аватарка пользователя"
                                ...
                                <div class="flex space-x-1">
                                    ...
                                    <form method="post" action="{{ route('comments.destroy', ['comment' => $comment]) }}">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="h-full w-full">
                                            <svg class="w-5 h-5 text-red-500 hover:text-red-400" viewBox="0 0 24 24"
                                                fill="currentColor">
                                                <path fill-rule="evenodd"
                                    ...
                                    <form method="get" action="{{ route('comments.edit', ['comment' => $comment]) }}">
                                        @csrf
                                        <input type="hidden" name="rewiew_id" value="{{ $rewiew->id }}" />
                                        <button type="submit" class="h-full w-full">
                                            <svg class="w-5 h-5 text-cyan-400 hover:text-cyan-300" viewBox="0 0 24 24"
                                                fill="currentColor">
                                                <path fill-rule="evenodd"
