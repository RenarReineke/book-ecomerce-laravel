@extends('admin.layouts.dashboard') @section('rewiewList')
    <x-page-list title="Отзывы" formUrl="{{ route('rewiews.create') }}" indexUrl="{{ route('rewiews.index') }}"
            ...
            @foreach ($rewiews as $rewiew)
                ...
                <div
                    ...
                    <div class="p-2 w-28 flex justify-start items-center">
                        @for ($i = 1; $i <= 5; $i++)
                            <svg class="w-8 h-8 stroke-none {{ $i <= $rewiew->rating ? 'fill-pink-500' : 'fill-slate-500' }}"
                    ...
                    <!-- Действия -->
                    <x-action-list detail-link="{{ route('rewiews.show', ['rewiew' => $rewiew]) }}"
