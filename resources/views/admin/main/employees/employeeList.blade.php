...
<div class="relative flex flex-row-reverse h-full bg-sky-50 border-4 rounded-lg">
    ...
    <x-filters url="{{route('employees.index')}}">
        <x-employee-filters></x-employee-filters>
    </x-filters>

    <!-- Main -->
    <div class="p-10 pt-4 px-5 h-auto w-full bg-sky-50">
        <!-- Верхняя панель -->       
            <x-header title="Сотрудники" formUrl="{{route('employees.create')}}" searchUrl="{{route('employees.index')}}"></x-header>
