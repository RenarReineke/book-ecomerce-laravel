...
<div class="max-w-md mx-auto">
    ...
    <h2 class="mt-1 text-2xl sm:text-xl font-bold text-slate-500">Редактировать категорию</h1>
</div>
...
<div class="p-6 sm:p-10 sm:pt-0 mx-auto max-w-md w-full bg-white/80 backdrop-blur-xl rounded-xl shadow-xl">
    <form action="{{route('categories.update', ['category' => $category])}}" method="post" novalidate class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="relative rounded-md shadow-sm mt-1">
                ...
                <input value="{{old('title')}}" type="text" id="title" name="title" required placeholder="Фэнтези" class="pl-10 w-full rounded-md text-sm {{ $errors->has('title') ? 'placeholder-red-300 border-red-300 focus:border-red-500 focus:ring-red-500' : 'placeholder:text-gray-400 border-gray-300 focus:border-cyan-500 focus:ring-cyan-500'}}">
                @error('title')
        ...
        <div>
            ...
            <div class="relative rounded-md shadow-sm mt-1">
                ...
                <input value="{{old('description')}}" type="text" id="description" name="description" required placeholder="Жанр фантастики без привязки к научному мировоззрению" class="pl-10 w-full rounded-md text-sm {{ $errors->has('description') ? 'placeholder-red-300 border-red-300 focus:border-red-500 focus:ring-red-500' : 'placeholder:text-gray-400 border-gray-300 focus:border-cyan-500 focus:ring-cyan-500'}}">
                @error('description')
                    <div class="pr-3 absolute inset-y-0 right-0 flex items-center">
                        <svg viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5 text-red-500">
                        <path fill-rule="evenodd" d="M11.484 2.17a.75.75 0 011.032 0 11.209 11.209 0 007.877 3.08.75.75 0 01.722.515 12.74 12.74 0 01.635 3.985c0 5.942-4.064 10.933-9.563 12.348a.749.749 0 01-.374 0C6.314 20.683 2.25 15.692 2.25 9.75c0-1.39.223-2.73.635-3.985a.75.75 0 01.722-.516l.143.001c2.996 0 5.718-1.17 7.734-3.08zM12 8.25a.75.75 0 01.75.75v3.75a.75.75 0 01-1.5 0V9a.75.75 0 01.75-.75zM12 15a.75.75 0 00-.75.75v.008c0 .414.336.75.75.75h.008a.75.75 0 00.75-.75v-.008a.75.75 0 00-.75-.75H12z" clip-rule="evenodd" />
