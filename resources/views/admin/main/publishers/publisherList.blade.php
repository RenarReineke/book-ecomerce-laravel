...
<h1 class="mx-auto mt-5 w-96 text-lg text-gray-700 text-center">
    Издатели
    <!-- Форма на создание издателя -->
    <form method="get" action="{{route('publishers.create')}}" class="mx-auto w-12">
        ...
        <tbody>
            ...
            <tr class="h-10 hover:bg-slate-200 hover:cursor-pointer">
                <td class="text-center border border-slate-300 pl-4">
                    ...
                    <form
                        method="post"
                        action="{{route('publishers.destroy', ['publisher' => $publisher])}}"
                    >
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="h-full w-full">
                ...
                <td class="p-2 text-center border border-slate-300">
                    ...
                    <a href="{{route('publishers.show', ['publisher' => $publisher])}}" class="block hover:bg-slate-700 hover:text-white">
                        {{$publisher->id}}
                    </a>
                </td>
                ...
                <td class="pl-4 text-center border border-slate-300">
                    <!-- Редактировать -->
                    <form method="get" action="{{route('publishers.edit', ['publisher' => $publisher])}}">
                        @csrf
                        <button type="submit" class="h-full w-full">
                            <svg
