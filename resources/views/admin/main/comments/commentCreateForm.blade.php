...
<div class="p-6 sm:p-10 sm:pt-0 mx-auto max-w-md w-full bg-white/80 backdrop-blur-xl rounded-xl shadow-xl">
    <form action="{{route('comments.store', ['rewiew_id' => $rewiew_id])}}" method="post" novalidate class="space-y-6">
        @csrf

        <!-- Преимущества -->
        <div>
            <div class="relative rounded-md shadow-sm mt-1">
                <textarea autofocus value="{{old('message')}}" type="text" id="message" name="message" required class="pl-2 w-full h-[120px] rounded-md text-slate-600 text-sm {{ $errors->has('message') ? 'placeholder-red-300 border-red-300 focus:border-red-500 focus:ring-red-500' : 'placeholder:text-gray-400 border-gray-300 focus:border-cyan-500 focus:ring-cyan-500'}}"></textarea>
                @error('message')
